...
namespace myGame
{
    internal class GameForm : Form
    {
        ...
        public GameForm(string map)
        {
            ...
            {
                Map = new Map(map, 50),
                Player = new Player(new Point { X = 75, Y = 121 }, 50, 10, 200, 10, new Dictionary<string, int>()),
                Enemies = new List<Enemy>() { new Enemy( new Point { X = 175, Y = 64 }, 100, 10, 50, 1, 1,
                new List<Point>() { new Point { X = 75, Y = 150 }, new Point { X = 175, Y = 150 }}, 50) }
            };
            ClientSize = new Size(600, 600);
            timer = new Timer { Interval = 10 };
            pos = new Label { Size = new Size(ClientSize.Width, 30) };
            Controls.Add(pos);
            DoubleBuffered = true;
        ...
        private void DrawMap(Graphics g)
        {
            for (var i = 0; i < game.Map.WidthInCells; i++)
                ...
                {
                    var brush = (game.Map.CellIsWall[i, j]) ? 
                        new SolidBrush(Color.Black) : 
                        new SolidBrush(Color.White);
                    g.FillRectangle(brush,
                        new Rectangle(i * game.Map.CellSize + 100,
                            j * game.Map.CellSize + 100,
